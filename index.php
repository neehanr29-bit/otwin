<?php
session_start();
include 'api/AmbildataBPS.php';
 
// Data total tahunan
$data_bps        = ambilDataBPS();
$data_terbaru    = end($data_bps);
$data_sebelumnya = $data_bps[count($data_bps) - 2] ?? null;
$pertumbuhan     = 0;
if ($data_sebelumnya && $data_sebelumnya['jumlah'] > 0) {
    $pertumbuhan = round((($data_terbaru['jumlah'] - $data_sebelumnya['jumlah']) / $data_sebelumnya['jumlah']) * 100, 1);
}
 
// Top 5 provinsi tujuan
$top_provinsi = ambilTopProvinsiTujuan();
$max_provinsi = !empty($top_provinsi) ? $top_provinsi[0]['jumlah'] : 1;
?>
<!DOCTYPE html>
<html lang="id">
<head>
  ...
  <div class="max-w-5xl mx-auto">
    ...
    <div class="reveal mb-10">
      ...
      <h2 class="font-playfair text-4xl text-dark leading-snug mb-2">
        Statistik Wisatawan Nusantara Indonesia
      </h2>
      <p class="text-muted text-sm">Data resmi perjalanan wisatawan domestik · diperbarui otomatis melalui API BPS.</p>
    </div>
 
    <!-- 3 Kartu Highlight -->
    <div class="reveal grid grid-cols-1 md:grid-cols-3 gap-5 mb-10">
      <div class="bg-teal rounded-2xl p-7 text-white">
        <div class="text-xs font-bold uppercase tracking-widest opacity-70 mb-2">Total Perjalanan <?= htmlspecialchars($data_terbaru['tahun']) ?></div>
        <div class="font-playfair text-4xl font-bold mb-1"><?= formatJumlah($data_terbaru['jumlah']) ?></div>
      ...
      <div class="bg-sand rounded-2xl p-7 border border-gold/20">
        ...
        <div class="font-playfair text-4xl font-bold mb-1 <?= $pertumbuhan >= 0 ? 'text-teal' : 'text-red-500' ?>">
          <?= ($pertumbuhan >= 0 ? '+' : '') . $pertumbuhan ?>%
        </div>
        <div class="text-xs text-muted">
          Dibanding <?= $data_sebelumnya ? htmlspecialchars($data_sebelumnya['tahun']) : '-' ?>
          (<?= $data_sebelumnya ? formatJumlah($data_sebelumnya['jumlah']) : '-' ?>)
        </div>
      </div>
      <div class="bg-sand rounded-2xl p-7 border border-gold/20">
        <div class="text-xs font-bold uppercase tracking-widest text-muted mb-2">Rentang Data Tersedia</div>
        <div class="font-playfair text-4xl font-bold text-gold mb-1"><?= count($data_bps) ?> Tahun</div>
        <div class="text-xs text-muted"><?= htmlspecialchars($data_bps[0]['tahun']) ?> – <?= htmlspecialchars($data_terbaru['tahun']) ?></div>
      </div>
    </div>
    ...
    <div class="reveal grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-gray-100 bg-cream">
              ...
              <th class="text-left px-5 py-3 text-[0.65rem] uppercase tracking-widest font-bold text-muted">Pertumbuhan</th>
            </tr>
          </thead>
          <tbody>
            <?php
            $max_j = max(array_column($data_bps, 'jumlah')) ?: 1;
            foreach ($data_bps as $idx => $row):
              $prev   = $idx > 0 ? $data_bps[$idx-1]['jumlah'] : null;
              $growth = ($prev && $prev > 0) ? round((($row['jumlah'] - $prev) / $prev) * 100, 1) : null;
              $pct    = round(($row['jumlah'] / $max_j) * 100);
            ?>
            <tr class="border-b border-gray-50 hover:bg-cream/60 transition-colors">
              <td class="px-5 py-3 font-bold text-dark"><?= $row['tahun'] ?></td>
              <td class="px-5 py-3 font-semibold text-teal text-xs"><?= formatJumlah($row['jumlah']) ?></td>
              ...
              <td class="px-5 py-3">
                <?php if ($growth !== null): ?>
                  <span class="text-xs font-bold px-2 py-0.5 rounded-full <?= $growth >= 0 ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-500' ?>">
                    <?= ($growth >= 0 ? '▲ +' : '▼ ') . $growth ?>%
                  </span>
                <?php else: ?>
      ...
      <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div class="p-6 flex flex-col gap-4">
          <?php foreach ($top_provinsi as $rank => $prov):
            $bar = $max_provinsi > 0 ? round(($prov['jumlah'] / $max_provinsi) * 100) : 0;
            $colors = ['bg-teal', 'bg-teal/80', 'bg-teal/60', 'bg-gold', 'bg-gold/70'];
            $color  = $colors[$rank] ?? 'bg-teal/40';
          ?>
          <div>
            <div class="flex items-center justify-between mb-1">
              <div class="flex items-center gap-2">
                <span class="text-xs font-bold text-muted w-4"><?= $rank+1 ?></span>
                <span class="text-sm font-semibold text-dark"><?= htmlspecialchars($prov['provinsi']) ?></span>
              </div>
              <span class="text-xs font-bold text-teal"><?= formatJumlah($prov['jumlah']) ?></span>
            </div>
            <div class="h-2.5 bg-gray-100 rounded-full overflow-hidden">
              <div class="h-full <?= $color ?> rounded-full transition-all" style="width:<?= $bar ?>%"></div>
            </div>
          </div>
          <?php endforeach; ?>
        </div>
    ...
    <div class="text-xs text-muted text-center py-2">
      📊 Data bersumber dari API resmi <strong>Badan Pusat Statistik RI (BPS)</strong> · webapi.bps.go.id · Var. 2201 & 2195
    </div>
...
<script>
  const IS_LOGGED_IN = <?= isset($_SESSION['username']) ? 'true' : 'false' ?>;
</script>
